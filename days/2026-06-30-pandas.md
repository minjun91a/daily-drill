...
*재도전 없음 — 오늘의 문제를 단계별로 짚어가며 그 자리에서 합격.*
...
- **문제**:
  ...
  실제 급여 합 = 1800.
- **내 예측 변천 (그대로)**:
  - 힌트 없이 1차: "merged 7행 / total 2300 / *left merge라서 모든 행이 더해짐* / 교정 `salary.sum()`" → ❌ (total 2300만 맞음 / 행수 7 오답 + 원인을 'how=left'로 회귀 = 오늘 오전 고친 함정 / `salary` 미정의 `NameError`)
...
- 개념은 잡았으나 **힌트 없이 풀 때 'how=left'로 한 번 회귀** → "오른쪽 키 유니크?" 자동 점검 습관화 필요.
