...
![days](https://img.shields.io/badge/Day-14-brightgreen)
...
| [13](days/2026-07-22-sql.md) | 2026-07-22 | SQL | **`LEFT JOIN`의 오른쪽 테이블 컬럼을 `WHERE`로 필터하면 바깥 조인이 `INNER`로 무너져** "없으면 0"이어야 할 고객이 통째로 증발 → 필터는 `ON`에 `AND`로(`WHERE`=사후 행 삭제, `ON`=붙일지 결정·왼쪽 보존) · 오답fix `OR IS NULL`은 짝 없는 경우(NULL)만 살리고 조건 미달값은 못 살림 · 교정 결과 예측서 `COUNT(컬럼)`은 NULL 안 셈(0)·"행 등장≠count값" 재확인 · **직접 구현**: 도전과제 집계 백지 작성(`ON` 필터+`COUNT(컬럼)`) 1회 합격 | ✅ |
| [14](days/2026-07-24-pandas.md) | 2026-07-24 | pandas | **불리언 인덱싱에서 `&`·`\|`는 `==`·`>=`·`<`보다, `&`는 `\|`보다 우선순위가 높음** → 괄호를 빼면 사람이 읽는 순서와 다르게 묶여 `TypeError`(필터 시작도 못 함) · 3단 규칙: **개별 조건 괄호 + 기호(`&`/`\|`) 연결 + 혼합 시 묶음 괄호** · `and`/`or`는 Series에 못 씀(`truth value is ambiguous`) · **직접 구현 3회 만에 합격** — 2회차가 **에러 없이** 재고 40짜리를 통과시킨 게 최대 교훈 | ✅ |
...
- **(pandas)** **불리언 인덱싱에서 `&`·`|`는 비교 연산자(`==`·`>=`·`<`)보다 우선순위가 높고, `&`는 `|`보다 높다.** 괄호를 빼면 파이썬이 사람이 읽는 순서와 **다르게** 묶는다 — `df[df['a']=='x' & df['b']>=10]`은 `df['a'] == ('x' & df['b']) >= 10`으로 묶여 `TypeError`(필터가 시작조차 못 함). 3단 규칙: ⑴ **개별 조건마다 각각 괄호**, ⑵ 연결은 단어가 아니라 **기호(`&`/`|`)**, ⑶ **`&`와 `|`를 섞으면 먼저 묶일 쪽에 괄호를 하나 더**. 빠진 위치에 따라 **터지거나(즉시 발각) 조용히 틀린 행을 통과**시킨다 — `(A) | (B) & (C)`는 `(A) | ((B) & (C))`가 되어 **A만 만족하면 C를 무시하고 통과**한다. 출력 행 수만 보고 넘기지 말고 **스펙과 한 행씩 대조**할 것.
- **(pandas)** **`and`/`or`/`not`은 Series에 쓸 수 없다** — `ValueError: The truth value of a Series is ambiguous`. 값 **하나**의 참·거짓을 묻는 연산이라 행이 여러 개면 성립하지 않는다. 행마다 각각 판단하려면 원소별 `&`/`|`/`~`.
...
- **(Python)** **`if not x:`는 "값이 없나?"가 아니라 "falsy인가?"를 묻는다** — `None`·`0`·`0.0`·`""`·`[]`·`{}`·`False`를 **전부 한 그물로** 잡는다. `0`이나 빈 컨테이너가 **정상 값일 수 있는 자리**(할인율 0%, 수신자 0명, 무료배송)에서 쓰면 그 값을 "없음"으로 오인해 기본값으로 **조용히 덮어쓴다**(에러가 없어 더 위험). "정말 안 준 경우"만 잡으려면 **`if x is None:`**. 반사신경: `if not x:`를 보면 "x가 0이면? 빈 값이면? 그게 정상 값일 수 있나?"
