...
## 🐧 Linux / OS 기초 트랙
코드 드릴과 **별도로** OS/Linux를 기초부터 레벨 순서(L0~L10)로. 같은 방법(예측→대조→검증→직접)에 각 명령마다 **개념 씨앗**을 얹는다. 환경은 **WSL2(진짜 우분투)** 목표, 임시로 Git Bash. → **로드맵·진행표: [`linux/`](linux/README.md)** · 트리거는 "오늘 리눅스".
...
| [`linux/`](linux/README.md) | Linux/OS 기초 트랙 (레벨별 기록 + 로드맵) |
